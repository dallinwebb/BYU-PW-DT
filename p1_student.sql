...
FROM 
    student s
    LEFT JOIN PERSON_EXTRA pe 
        ON pe.PERSON_ID = s.PERSON_ID
    LEFT JOIN person_program pp 
        ON pp.person_id = s.person_id
    LEFT JOIN manage_offer mo 
        ON mo.person_program_id = pp.person_program_id
    LEFT JOIN PERSON_STATUS ps 
        ON ps.PERSON_STATUS_ID = pe.PERSON_STATUS_ID;
